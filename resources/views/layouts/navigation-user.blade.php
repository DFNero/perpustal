<div x-data="{ sidebarOpen: window.innerWidth > 1024 }" 
    ...
    >
        ...
        <nav class="flex-1 overflow-y-auto px-3 py-6 space-y-1">
            ...
            <!-- Daftar Buku (Active State Logic) -->
            <a href="{{ route('books.index') }}" class="relative flex items-center gap-3 px-3 py-3 rounded-lg group transition-all duration-200 {{ request()->routeIs('books.*') ? 'bg-orange-600 text-white shadow-lg shadow-orange-900/50' : 'hover:bg-gray-800 text-gray-400 hover:text-white' }}">
                <!-- Book Icon -->
                <svg xmlns="http://www.w3.org/2000/svg" width="22" height="22" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="shrink-0"><path d="M4 19.5v-15A2.5 2.5 0 0 1 6.5 2H20v20H6.5a2.5 2.5 0 0 1 0-5H20"/></svg>
                <span x-show="sidebarOpen" class="font-medium whitespace-nowrap">Daftar Buku</span>
            </a>

            <!-- My Borrowings -->
            <a href="{{ route('borrowings.index') }}" class="relative flex items-center gap-3 px-3 py-3 rounded-lg group transition-all duration-200 {{ request()->routeIs('borrowings.*') ? 'bg-orange-600 text-white' : 'hover:bg-gray-800 text-gray-400 hover:text-white' }}">
                <svg xmlns="http://www.w3.org/2000/svg" width="22" height="22" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="shrink-0"><path d="M16 21v-2a4 4 0 0 0-4-4H5a4 4 0 0 0-4 4v2"/><circle cx="8.5" cy="7" r="4"/><line x1="20" y1="8" x2="20" y2="14"/><line x1="23" y1="11" x2="17" y2="11"/></svg>
                <span x-show="sidebarOpen" class="font-medium whitespace-nowrap">Peminjaman Saya</span>
                <div x-show="!sidebarOpen" x-transition.opacity class="absolute left-full ml-3 z-50 w-max px-2 py-1 bg-gray-900 text-white text-xs rounded-md shadow-lg">Peminjaman</div>
            </a>

            <!-- Notifications -->
            <a href="{{ route('notifications.index') }}" class="relative flex items-center gap-3 px-3 py-3 rounded-lg group transition-all duration-200 {{ request()->routeIs('notifications.*') ? 'bg-orange-600 text-white' : 'hover:bg-gray-800 text-gray-400 hover:text-white' }}">
                <svg xmlns="http://www.w3.org/2000/svg" width="22" height="22" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="shrink-0"><path d="M6 8a6 6 0 0 1 12 0c0 7 3 9 3 9H3s3-2 3-9"/><path d="M10.3 21a1.94 1.94 0 0 0 3.4 0"/></svg>
                <span x-show="sidebarOpen" class="font-medium whitespace-nowrap">Notifikasi</span>
                @if (auth()->user()->unreadNotifications->count())
                    <span x-show="sidebarOpen" class="ml-auto w-2 h-2 bg-red-500 rounded-full animate-pulse"></span>
                @endif
                <div x-show="!sidebarOpen" x-transition.opacity class="absolute left-full ml-3 z-50 w-max px-2 py-1 bg-gray-900 text-white text-xs rounded-md shadow-lg">Notifikasi</div>
            </a>

            <!-- Activity Log -->
            <a href="{{ route('activity-log.index') }}" class="relative flex items-center gap-3 px-3 py-3 rounded-lg group transition-all duration-200 {{ request()->routeIs('activity-log.*') ? 'bg-orange-600 text-white' : 'hover:bg-gray-800 text-gray-400 hover:text-white' }}">
        ...
        <div class="p-4 border-t border-gray-700 shrink-0 bg-gray-800">
            <div class="flex items-center gap-3">
                <div class="w-8 h-8 rounded-full bg-orange-600 flex items-center justify-center text-white font-bold text-xs ring-2 ring-gray-700">
            ...
            <div class="flex items-center gap-3">
                ...
                <div class="relative" x-data="{ open: false }">
                    ...
                    <div x-show="open" @click.outside="open = false" x-transition class="absolute right-0 mt-2 w-80 bg-white shadow-lg rounded-lg border z-50">
                        ...
                        @empty
                            <div class="p-4 text-sm text-gray-500 text-center">Tidak ada notifikasi baru</div>
                        @endforelse
                        @if (auth()->user()->unreadNotifications->count())
                ...
                <div class="relative group">
                    <button class="flex items-center gap-2 focus:outline-none">
                        <div class="text-right hidden md:block">
                            ...
                            <p class="text-[10px] text-gray-500 mt-1">Profile</p>
                        </div>
