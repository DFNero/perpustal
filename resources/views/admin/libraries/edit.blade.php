    ...
    <div class="max-w-2xl mx-auto bg-white p-6 rounded-lg shadow">
        ...
        <form action="{{ route('admin.libraries.update', $library) }}" method="POST" class="space-y-4">
            ...
            <div>
                ...
                <select name="city_id" class="w-full border border-gray-300 px-3 py-2 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 @error('city_id') border-red-500 @enderror" required>
                    ...
                    @foreach($cities as $city)
                        <option value="{{ $city->id }}" {{ old('city_id', $library->city_id) == $city->id ? 'selected' : '' }}>
                            {{ $city->name }}
                        </option>
                    @endforeach
                </select>
            ...
            <div>
                ...
                <input type="text" name="name" value="{{ old('name', $library->name) }}" class="w-full border border-gray-300 px-3 py-2 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 @error('name') border-red-500 @enderror" required>
                @error('name')
                    <span class="text-red-600 text-sm">{{ $message }}</span>
                @enderror
            </div>
            ...
            <div>
                ...
                <textarea name="address" class="w-full border border-gray-300 px-3 py-2 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 h-20 @error('address') border-red-500 @enderror" required>{{ old('address', $library->address) }}</textarea>
                @error('address')
                    <span class="text-red-600 text-sm">{{ $message }}</span>
                @enderror
            </div>
            ...
            <div class="grid grid-cols-2 gap-4">
                <div>
                    ...
                    <input type="text" name="latitude" value="{{ old('latitude', $library->latitude) }}" class="w-full border border-gray-300 px-3 py-2 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 @error('latitude') border-red-500 @enderror" required>
                ...
                <div>
                    ...
                    <input type="text" name="longitude" value="{{ old('longitude', $library->longitude) }}" class="w-full border border-gray-300 px-3 py-2 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 @error('longitude') border-red-500 @enderror" required>
